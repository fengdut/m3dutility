...
void read_xyz(settype  settings,hid_t h5fid,int frame,hdatatype *hdata,double *parity ,hsize_t  *npoints)
{
	...
	if (settings.plane0 >= hdata->planes[0]) 
	{
	settings.plane0 = settings.plane0 % hdata->planes[0];
	fprintf(stderr, "Plane requested was out of range; resetting to %d.\n",settings.plane0);
	}
	...
	for (var=0; var<nvars; var++) 
	{
		...
		if (veclen == 3) 
		{
			...
			if (*fname == 'B') 
			{
				...
				/* Filter, if desired */
				if (settings.filtermode >= 0) 
				{
					if (settings.filtermode)
						fprintf(stderr, "Filtering; leaving modes 0 and %d.\n",settings.filtermode);
					else 
						fputs("Filtering; leaving mode 0 only.\n", stderr);

					filter(hdata->BR, settings.filtermode, hdata);
					filter(hdata->Bphi, settings.filtermode, hdata);
					filter(hdata->Bz, settings.filtermode, hdata);
				}
